...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt->num_rows == 0) {
        ...
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ssisisssisis", $fechaActual, $horaActual, $idDirector, $tipoId, $idCortesia, $notificacion, $fechaActual, $horaActual,  $idSupervisor, $tipoId, $idCortesia, $notificacion);
        $stmt->execute();
    } else {
        $stmt->bind_result($id);
        $stmt->fetch();
