                    ...
                    <tbody>
                        ...
                        if ($idEntregado == 4) {
                            $queryDirector = "SELECT a.id, fecha, hora, b.nombreCompleto As entregado, c.nombreCompleto AS supervisor, bs, usd, eur, estatus
                                            ...
                                            WHERE ((a.estatus='Enviado a Director' OR a.estatus LIKE 'Redirigido a Director%')  
                                            AND idEntregado = $idEntregado) OR a.estatus='En espera de autorización'
                                            ORDER BY a.id DESC;";
                        } else {
                            $queryDirector = "SELECT a.id, fecha, hora, b.nombreCompleto As entregado, c.nombreCompleto AS supervisor, bs, usd, eur, estatus
                                            FROM trazabilidad_conglomerado a 
                        ...
                        $queryTesoreria = "SELECT a.id, fecha, hora, b.nombreCompleto As entregado, c.nombreCompleto AS supervisor, bs, usd, eur, estatus
                                            ...
                                            JOIN trazabilidad_usuario b ON  a.idEntregado=b.id
                                            JOIN trazabilidad_usuario c ON  a.idSupervisor=c.id
                                            WHERE estatus='Redirigido a Tesorería' OR estatus='Autorizado por Director'
                                            ORDER BY a.id DESC;";
                        ...
                        if ($stmt = $conn->prepare($query)) {
                            ...
                            while ($row = $result->fetch_assoc()) {
                                ...
                                $botonRedirigir = $estatus == 'Redirigido a Tesorería' || str_contains($estatus, 'Redirigido a Director') ? '' : '<a id= "btnRedirigir' . htmlspecialchars($row['id']) . '" class="btn btn-warning mt-1" data-toggle="modal" data-target="#redirigirModal" onclick="modalRedirigirEntregaRecibo(' . htmlspecialchars($row['id']) . ')">Redirigir</a>';
                ...
                <div class="container" style="min-width: 100%;">
                    ...
                    <form id="formRedirigirRecibo" onsubmit="formRedirigirRecibo()">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                <select class="form-select form-control" id="selectTipo" name="tipo" onchange="mostrarInputs();" required>
                        ...
                        <div id="divBs" class="form-group row" hidden>
                            <label for="bs" class="col-sm-3 col-form-label">Bolívares</label>
                            <div class="col-sm-8">
                                <input type="number" class="form-control" id="inputBs" name="bs" min="0" onchange="validarMax('inputBs');" autocomplete="off">
                            </div>
                        </div>
                        <div id="divUsd" class="form-group row" hidden>
                            <label for="usd" class="col-sm-3 col-form-label">Dólares</label>
                            <div class="col-sm-8">
                                <input type="number" class="form-control" id="inputUsd" name="usd" min="0" onchange="validarMax('inputUsd');" autocomplete="off">
                            </div>
                        </div>
                        <div id="divEur" class="form-group row" hidden>
                            <label for="eur" class="col-sm-3 col-form-label">Euros</label>
                            <div class="col-sm-8">
                                <input type="number" class="form-control" id="inputEur" name="eur" min="0" onchange="validarMax('inputEur');" autocomplete="off">
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                <select class="form-select form-control" id="redirigir" name="redirigir" required>
                                    <option value="">--</option>
                                    <?php
                                    $iduser = $_SESSION['idUser'];
                                    $query = "SELECT id, rol, nombreCompleto FROM trazabilidad_usuario WHERE (rol = 'Tesoreria' OR rol = 'Director') AND id <> $iduser ORDER BY nombreCompleto;";
                                    echo $query;
...
<script>
    ...
    function modalConfirmarEntregaRecibo(id) {
        ...
        fetch(url)
            ...
            .then(conglomerado => {
                $("#confirmarModalLabel").text(`Conglomerado Nº ${id} - ${conglomerado.estatus}`);
    ...
    function modalAutorizarEntregaRecibo(id) {
        ...
        fetch(url)
            ...
            .then(conglomerado => {
                ...
                $("#autorizarModalLabel").text(`Conglomerado Nº ${id} - ${conglomerado.estatus}`);
    ...
    function modalRedirigirEntregaRecibo(id) {
        ...
        fetch(url)
            ...
            .then(conglomerado => {
                ...
                $('#inputBs').attr('max', conglomerado.bs);
                $('#inputUsd').attr('max', conglomerado.usd);
                $('#inputEur').attr('max', conglomerado.eur);
    ...
    function validarMax(idInput) {
        const input = $(`#${idInput}`);
        const maximo = input.attr('max');
        const valorIngresado = Number(input.val());

        if (valorIngresado > maximo) {
            alert(`El valor ingresado debe ser menor o igual a ${maximo}`);
            input.val(maximo);
        }
    }
    ...
    function mostrarInputs() {
        ...
        if (tipo == 'p') {
            $('#divBs').attr('hidden', false);
            $('#divUsd').attr('hidden', false);
            $('#divEur').attr('hidden', false);

            $('#inputBs').attr('required', true);
            $('#inputUsd').attr('required', true);
            $('#inputEur').attr('required', true);

        } else {
            $('#divBs').attr('hidden', true);
            $('#divUsd').attr('hidden', true);
            $('#divEur').attr('hidden', true);

            $('#inputBs').attr('required', false);
            $('#inputUsd').attr('required', false);
            $('#inputEur').attr('required', false);
        }
