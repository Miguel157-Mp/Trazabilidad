    ...
    prioridad p ON s.idPrioridad = p.idPrioridad;";
                        ...
                        if ($result->num_rows > 0) {
                            ...
                            while ($row = $result->fetch_assoc()) {
                                ...
                                echo '<td>' . $row["idSolicitud"] . ' | ' . $row["nombreTienda"] . '</td>';
                                echo '<td>
                                <div class="col-sm-10">
                                    <select type="text" class="form-control" id="prioridadSugerida-' . $row['idSolicitud'] . '" name="prioridadSugerida" required autocomplete="off" onchange="guardarPrioridad(' . $row['idSolicitud'] . ')">
                                        <option value="">Seleccione una opción</option>
                                        <option value="1"' . (($row['prioridadSugerida'] == 1) ? ' selected' : '') . '>🔴 Critico</option>
                                        <option value="2"' . (($row['prioridadSugerida'] == 2) ? ' selected' : '') . '>🟠 Alto</option>
                                        <option value="3"' . (($row['prioridadSugerida'] == 3) ? ' selected' : '') . '>🟢 Medio</option>
                                        <option value="4"' . (($row['prioridadSugerida'] == 4) ? ' selected' : '') . '>⚪ Bajo</option>
                                    </select>
                                </div>
                              </td>';

                                echo '<td>' . $row["nombrePrioridad"] . '</td>';
...
<script>
function guardarPrioridad(idSolicitud) {
    var prioridad = document.getElementById('prioridadSugerida-' + idSolicitud).value;

    if (prioridad) {
        var xhr = new XMLHttpRequest();
        xhr.open("POST", "./scripts/guardar_prioridad.php", true);
        xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 200) {
                alert("Prioridad guardada exitosamente");
            }
        };
        xhr.send("idSolicitud=" + idSolicitud + "&prioridadSugerida=" + prioridad);
    } else {
    ...
    function markAsRead(idSolicitud) {
        ...
        var xhr = new XMLHttpRequest();
        xhr.open("POST", "./content/actualizarLeido.php", true);
        xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
