    ...
    <div class="card shadow mb-4">
                    ...
                    <tbody>
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                $imgPresupuesto = $row['imgPresupuesto'];
                                $idSolicitud = htmlspecialchars($row['idSolicitud']);
     ...
     <td>   <div class="circle1 " style="background-color:' . $color . '; color: white;  ">' . htmlspecialchars($row['']) . ' ' . htmlspecialchars($row['nombrePrioridad']) . '</div></td>
            <td>
               ...
               <td>
            <form id="formCrearRecibo_' . $idSolicitud . '" onsubmit="crearRecibo(event)" action="./scripts/trazabilidadSubirImagenFinalizado.php" method="post" enctype="multipart/form-data">
                <input type="hidden" name="idSolicitud" value="' . $idSolicitud . '">';

                                if ($imgPresupuesto === NULL) {
                                    echo '
                <label for="imgPresupuesto_' . $idSolicitud . '">Selecciona una imagen:</label>
                <input  id="imgPresupuesto_' . $idSolicitud . '" name="imgPresupuesto" type="file" required>
                <br>
                <button class="btn btn-secondary" type="submit">Subir Imagen</button>';
                                } else {
                                    echo '<p>La imagen ya ha sido subida.</p>'; // Mensaje alternativo si la imagen no es NULL
                                }
...
<script>
    ...
    function crearRecibo(event) {
        try {
            ...
            const form = event.target; // 'event.target' se refiere al formulario que disparó el evento

            // Crear un FormData para gestionar el archivo
            var formData = new FormData(form); // Aquí usamos el formulario directamente

            // Obtener el valor de idSolicitud desde el campo oculto
            const idSolicitud = form.querySelector('input[name="idSolicitud"]').value; // Usamos 'form' para obtener el valor

            const fileInput = form.querySelector('input[name="imgPresupuesto"]'); // Usamos 'form' para obtener la entrada de archivo
            ...
            reader.onloadend = function (e) {
                ...
                const formdata = {
                    ...
                    imgPresupuesto: base64String, // Aquí se agrega el archivo en Base64
                };

                // Enviar datos a trazabilidadSubirImagenFinalizado.php
                $.ajax({
                    url: './scripts/trazabilidadSubirImagenFinalizado.php',
                    type: 'POST',
                    ...
                    success: function (response) {
                        ...
                        if (response.estatus) {
                            ...
                            form.reset(); // Resetea solo este formulario
