...
<div class="container-fluid">
    ...
    <p class="mb-4">Sin requerimientos</p>
        ...
        <div class="card-body">
            ...
            <form id="formCrearRecibo" onsubmit="crearRecibo()">
                <div class="form-group row">
                    <label for="nombreTienda" class="col-sm-2 col-form-label">Tienda | Requerimiento | Reparación</label>
                    ...
                    <div class="col-sm-10">
                        <select class="form-select form-control" id="nombreTienda" name="nombreTienda"
                            ...
                            <?php
    // $query = "SELECT t.idTienda, t.nombreTienda, t.email, a.idTienda AS idTiendaAdmin, a.requerimiento, a.nombreTienda, a.idSolicitud FROM tienda t LEFT JOIN solicitud a ON t.idTienda = a.idTienda WHERE leido = '1';";
    $query = " SELECT t.idTienda, t.nombreTienda, t.email, a.idTienda AS idTiendaAdmin, a.requerimiento, a.nombreTienda, a.idSolicitud 
    FROM tienda t 
    LEFT JOIN solicitud a ON t.idTienda = a.idTienda 
    WHERE leido = '1' 
    AND a.idSolicitud NOT IN (SELECT idSolicitud FROM administracion);
    ";            
                            ...
                            if ($stmt->execute()) {
                                ...
                                while ($row = $result->fetch_assoc()) {
                                    echo '<option value="' . htmlspecialchars($row['idTienda']) . '" data-idtienda="' . htmlspecialchars($row['idTienda']) . '" data-idsolicitud="' . htmlspecialchars($row['idSolicitud']) . '">'
                                        . htmlspecialchars($row['nombreTienda']) . ' | ' . htmlspecialchars($row['idSolicitud']) . ' | ' . htmlspecialchars($row['requerimiento']) . '</option>';
                                }
                            }
                            ?>
                        </select>
